...
def fetch_and_send_articles():
    url = 'https://newsapi.org/v2/top-headlines?country=us&category=science&apiKey=a61ddae6d1f04e839adb6268a8451c28'
    response = requests.get(url)
    if response.status_code == 200:
        articles = response.json().get('articles', [])
        for article in articles:
            producer.send("tech-news", article)
            #send_to_kafka('tech-news', article)
